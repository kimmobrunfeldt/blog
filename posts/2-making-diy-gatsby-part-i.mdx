...
[Gatsby](https://www.gatsbyjs.com/) is a production-ready tool that has everything you need: customisable data sources accessed via GraphQL, optimised build pipeline, functionality via plugins, and huge community.
...
Building on top of the previous example, let's add a JS bundle called `hydrate.js` script that the browser will execute. It uses [ReactDOM.hydrate()](https://reactjs.org/docs/react-dom.html#hydrate) to continue where the server-side rendered React left off:
...
That's the rough idea. There's a ton of details that still need to be solved though. In the [Part II](../3-diy-gatsby-part-ii/) we'll look into module bundling and Rollup.
