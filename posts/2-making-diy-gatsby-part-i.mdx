...
description: >
  The first post of a series where we look into how the do-it-yourself static site generation works for kimmo.blog.
tags:
...
When a user loads `/posts/` path in their browser, the server responds with contents of `output/posts/index.html` file. We want the actual page content to already be in the HTML already at that stage. It will make the content load faster, more SEO friendly, and accessible even when JS has been disabled.
