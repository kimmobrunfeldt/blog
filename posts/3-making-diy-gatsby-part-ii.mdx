...
**Render the page-specific `hydrate.tsx`** using [pageHydrate.tsx.template](https://github.com/kimmobrunfeldt/blog/blob/main/src/templates/pageHydrate.tsx.template).
...
With these building blocks, we are able to convert React pages into static HTML counter parts. To recap, `npm run build` will:
...
The build flow is simple in theory, but there are a huge amount of details that end up making the process complicated. If this wasn't complicated enough for you, Part III will cover how MDX pages are rendered.
