...
1. (11.8s) `Render pages` stage. Generates entrypoint files for each page
    ...
    * For each "regular" page under [src/pages]((https://github.com/kimmobrunfeldt/blog/blob/main/src/pages)
        * `index.html` the main HTML for the page.
        * `hydrate.tsx` the script that `React.hydrate`s the given page when loaded at frontend

            [src/pages/_exports.ts]((https://github.com/kimmobrunfeldt/blog/blob/main/src/pages/_exports.ts) has information that tells which
            React component file we should import (e.g. Index.tsx).

            The static parts of the page work just well without this script.

    * For each MDX "post" page under [posts]((https://github.com/kimmobrunfeldt/blog/blob/main/posts)
...
1. `Static files` stage. Copies everything from [public/]((https://github.com/kimmobrunfeldt/blog/blob/main/public/) to the root of output
