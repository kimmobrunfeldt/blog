...
Turns out I wanted to code a bit more — at least in the beginning. This blog is the definition of over-engineering, considering the goal was to write. But it's not that simple. Even though I had a measurable goal, a side project shouldn't feel like work either. For me, implementing the site from scratch felt like a necessary fuel for my writing inspiration. I'll try to explain.
...
The goal of this blog is to share my thoughts and ideally help other people. To achieve that, there's no need to track users, ask if they want to receive notifications via service workers, ask their location, or anything like that. There's just the content in a minimal digital frame.
...
After a lot of initial hesitation between do-it-yourself or `npm i gatsby`, I decided to `git init` and started coding a Gatsby-like static site generator. It sounded like a huge time sink but I wanted to know how minimal it can get. The benefits of a custom static site generator compared to Gatsby would be most probably microscopical, but why not? Reinventing a thing is a good learning experience. It would also provide answers to a few questions I've been wondering.

> How does Gatsby or React Static work?
> Could the page bundle be even smaller?
>
> &nbsp;
> How hard can it be to just renderToString and hydrate?

Well it can be hard that's for sure, but I'm really happy with the journey. The following posts will contain more details about the actual solution.
