...
That out of the way, let's jump into building the thing. The post will cover SSR basics, but even the seasoned developers should find some new interesting details.

The goal isn't to build a better Gatsby but to understand how it works and possibly make the site's bundle footprint even smaller. The high-level idea is pretty straightforward: convert a bunch of source files and content to a static site in `output/` directory.

<Presentic alt="Build pipeline overview presentation" duration={1200} src="/content-assets/2/build-pipeline-overview.svg" />

We ["simply"](https://www.knowledgeowl.com/home/dont-say-simply-jim-fisher) need to implement the _Build pipeline_ box in the middle. One way to start figuring out a system is from the user's perspective. Let's imagine there's a file server that serves everything under `output/` directory.
...
That works, but it's not as convenient. It's missing some of the ergonomics I find pleasant in modern web development. Anyways, that's beside the topic of this post. Let's get back to building the React-based static site generation.

<Image maxWidth={550} alt="Figure of another GET request" src="/content-assets/2/get-request-hydrate.svg" />

Building on top of the example above, this time the returned HTML refers to a `hydrate.js` script that it'll execute in the browser.
