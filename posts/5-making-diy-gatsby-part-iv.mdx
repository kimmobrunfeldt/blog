...
const promises = files.map((name) => fs.promises.readFile(name, {
  encoding: 'utf8'
}));
...
The final site builds ended up being quite similar in size. Gatsby wins in end-user performance by a small margin since it supports frontend routing. The routing allows faster page changes after initial load.
