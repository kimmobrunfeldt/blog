...
The argument might not be a fair one, since it's completely understandable why Gatsby does this. They rely on GraphQL as the single tech choice for accessing data. It's a smart choice for the average use case – accessing CMS data via GraphQL is very convenient.

## Good #3: No plugin overuse

Gatsby has over [2500 plugins](https://www.gatsbyjs.com/plugins). Many of them are tremendous and greatly simplify otherwise complicated workflow. For example `gatsby-plugin-image` which adds responsive images automatically on your website.
