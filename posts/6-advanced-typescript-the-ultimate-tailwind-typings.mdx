...
1. Support for a string type
1. Support for an object type
1. Support for an array type
...
Let's turn the pieces above into the final type utility which validates that each token is a valid Tailwind class in a string. The utility only works if the string is a literal type such as `"p-1 block"` instead of the general `string` type.
```
