...
*This is the Part&nbsp;I of a series where we'll look into how kimmo.blog works. Part&nbsp;I covers SSR and static site basics. If you are familiar with the topic, feel free to jump into [Part II: Module bundling](../3-diy-gatsby-part-ii/).*
...
In static sites though, each page needs to be able to independently bootstrap the JavaScript UI. In other words static sites can have multiple entrypoints into the application.

I wouldn't say there's a clear winner, just different approaches with varying pros and cons. Also, I'm going to conveniently just skip the fact that the custom build system doesn't support frontend routing. That will make navigation after initial page load slightly slower than in SPAs. It was a shortcut that I took to make the setup less complex. Hats off to Gatsby for actually pulling it off reliably.
