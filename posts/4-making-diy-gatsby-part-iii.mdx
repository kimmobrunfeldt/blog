...
*This is the Part&nbsp;III of a series where we'll look into how kimmo.blog works. Part&nbsp;III covers MDX rendering.*
...
While MDX is an incredible authoring experience, it comes at the price of complexity when done right. By right, I mean with good performance. MDX text could be sent as a raw string to the browser and it would do the heavy lifting with [@mdx-js/runtime](https://mdxjs.com/advanced/runtime#mdx-jsruntime), but that's not recommended:
